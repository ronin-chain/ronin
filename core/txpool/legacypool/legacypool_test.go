...
func TestMinGasPriceEnforced(t *testing.T) {
	...
	pool.Init(txPoolConfig.PriceLimit, blockchain.CurrentBlock().Header(), newReserver())
	defer pool.Close()
...
func TestUnderpricing(t *testing.T) {
	...
	if err := pool.AddRemote(pricedTransaction(5, 100000, big.NewInt(6), keys[1])); !errors.Is(err, ErrFutureReplacePending) {
...
func TestSetCodeTransactions(t *testing.T) {
	...
	defer pool.Close()
	pool.Init(testTxPoolConfig.PriceLimit, blockchain.CurrentBlock().Header(), newReserver())
