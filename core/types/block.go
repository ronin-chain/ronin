...
func (h *Header) EmptyBody() bool {
	...
	return h.TxHash == EmptyRootHash && h.UncleHash == EmptyUncleHash && emptyWithdrawals
}
...
// and receipts.
func NewBlock(header *Header, txs []*Transaction, uncles []*Header, receipts []*Receipt, hasher TrieHasher, withdrawals []*Withdrawal) *Block {
...
func CopyHeader(h *Header) *Header {
	...
	if h.WithdrawalsHash != nil {
		withdrawalHash := *h.WithdrawalsHash
		cpy.WithdrawalsHash = &withdrawalHash
	}
	return &cpy
}
...
func (b *Block) WithBody(transactions []*Transaction, uncles []*Header, withdrawals []*Withdrawal) *Block {
	// TODO maybe need to update this to add withdrawals as well, but let's decide after the dry-run
	block := &Block{
		header:       CopyHeader(b.header),
		transactions: make([]*Transaction, len(transactions)),
		uncles:       make([]*Header, len(uncles)),
		withdrawals:  slices.Clone(withdrawals),
	}
