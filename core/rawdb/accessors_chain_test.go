...
func TestBlockReceiptStorage(t *testing.T) {
	...
	// Create a minimal header for the test
	header := &types.Header{
		Number: big.NewInt(0),
		Time:   12, // Set a test timestamp
	}
	// Use the actual hash of the header
	hash := header.Hash()
...
func TestReadLogs(t *testing.T) {
	...
	if err := types.Receipts(receipts).DeriveFields(params.TestChainConfig, hash, 0, 12, nil, body.Transactions); err != nil {
		t.Fatal(err)
	}
...
func TestDeriveLogFields(t *testing.T) {
	...
	types.Receipts(receipts).DeriveFields(params.TestChainConfig, hash, number.Uint64(), blockTime, blobGasPrice, txs)
